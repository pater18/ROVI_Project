...
from sdurws import *
from sdurw import *
from sdurw_geometry import PointCloud
from sdurw_common import *
...
class Plugin(rwsplugin):
    def __init__(self,link):
        ...
        # initialize
        wc = WorkCellLoaderFactory.load("/home/student/Desktop/Project_WorkCell/Scene.wc.xml")
    ...
    def printProjectionMatrix(self, frameName):
        ...
        if (cameraFrame is not None):
            if (cameraFrame.getPropertyMap().has("Camera")):
                ...
                openGLToVis = Transform3Dd(RPYd(-math.pi,0,math.pi).toRotation3D())
                H = inverse(camPosOGL*inverse(openGLToVis))
    ...
    def createPathRRTConnect(self, From, To , extend, maxTime):
        ...
        super().getRobWorkStudio().setState(self.state)
        detector    = sdurw_proximity.ownedPtr(CollisionDetector(self.wc, PSF.makeDefaultCollisionStrategy()))
