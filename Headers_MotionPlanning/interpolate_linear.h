...
std::vector<rw::math::Q> getQConfigsLinear(rw::kinematics::MovableFrame::Ptr frameGoal, rw::models::SerialDevice::Ptr robot, rw::models::WorkCell::Ptr wc, rw::kinematics::State state)
{
...
int interpolateLinear()
{
	...
	for (int k = 0; k < 5; k++)
	{
		for (size_t i = 0; i < bottle_placements.size(); i++)
		{
			...
			std::vector<rw::math::Q> solutions_end = getQConfigsLinear(bottleFrame, robotUR5, wc, state);
			...
			std::vector<rw::math::Q> solutions = getQConfigsLinear(bottleFrame, robotUR5, wc, state);
