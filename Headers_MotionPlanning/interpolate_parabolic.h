...
std::vector<rw::math::Q> getQConfigsPara(rw::kinematics::MovableFrame::Ptr frameGoal, rw::models::SerialDevice::Ptr robot, rw::models::WorkCell::Ptr wc, rw::kinematics::State state)
{
...
void interpolateBetweenPointsPlanning(rw::trajectory::LinearInterpolator<rw::math::Q> interpolated_points, QPath &qpath_interpolated, std::vector<rw::math::Q> &x, std::vector<rw::math::Q> &dx, std::vector<rw::math::Q> &ddx, rw::models::SerialDevice::Ptr robot, rw::trajectory::TimedStatePath &tStatePath, double &current_time, rw::kinematics::State state)
{
	...
	for (float i = 0.0; i < interpolated_points.duration(); i += 0.01)
	{
        ...
        x.push_back(interpolated_points.x(i));
        dx.push_back(interpolated_points.dx(i));
        ddx.push_back(interpolated_points.ddx(i));
		tStatePath.push_back(rw::trajectory::TimedState(current_time, state));
		current_time += 0.01;
	}
}
void interpolateBetweenPointsPara(rw::trajectory::ParabolicBlend<rw::math::Q> interpolated_points, QPath &qpath_interpolated, std::vector<rw::math::Q> &x, std::vector<rw::math::Q> &dx, std::vector<rw::math::Q> &ddx, rw::models::SerialDevice::Ptr robot, rw::trajectory::TimedStatePath &tStatePath, double &current_time, rw::kinematics::State state)
{
...
int interpolatePara()
{
	...
	for (int k = 0; k < 5; k++)
	{
		for (size_t i = 0; i < bottle_placements.size(); i++)
		{
			...
			std::vector<rw::math::Q> solutions_end = getQConfigsPara(bottleFrame, robotUR5, wc, state);
			...
			std::vector<rw::math::Q> solutions = getQConfigsPara(bottleFrame, robotUR5, wc, state);
			...
			if (!collisionFreeSolutions.empty())
			{
                ...
                rw::trajectory::LinearInterpolator<rw::math::Q> interpolated_pointsSt1_transform(start_q, interpolation_step1, 1);
				...
				interpolateBetweenPointsPlanning(interpolated_points2tBottle, qpath_interpolated, x, dx, ddx, robotUR5, tStatePath, time, state);
				...
				interpolateBetweenPointsPlanning(interpolated_points5tEnd, qpath_interpolated, x, dx, ddx, robotUR5, tStatePath, time, state);
