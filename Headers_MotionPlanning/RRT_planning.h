...
std::vector<rw::math::Q> getQConfigsRRT(rw::kinematics::MovableFrame::Ptr frameGoal, rw::models::SerialDevice::Ptr robot, rw::models::WorkCell::Ptr wc, rw::kinematics::State state)
{
...
void interpolateBetweenPointsRRT(rw::trajectory::LinearInterpolator<rw::math::Q> interpolated_points, rw::models::SerialDevice::Ptr robot, rw::trajectory::TimedStatePath &tStatePath, double &current_time, rw::kinematics::State state)
{
...
void RRTp2pPlanning(rw::math::Q from, rw::math::Q to, double &total_length, double &total_time, rw::models::SerialDevice::Ptr robot, rw::proximity::CollisionDetector::Ptr detector, rw::trajectory::TimedStatePath &tStatePath, double &current_time, rw::kinematics::State state, rw::kinematics::Frame* frameTcp)
{
	typedef rw::math::Q V;
	...
	for (size_t i = 0; i < rrt_points.size() - 1; i++)
	{
		...
		interpolateBetweenPointsRRT(interpolated_points, robot, tStatePath, current_time, state);
	}
...
int rrtPlanning()
{
	...
	for (int k = 0; k < num_experiments; k++)
	{
		...
		for (size_t i = 0; i < bottle_placements.size(); i++)
		{
			...
			std::vector<rw::math::Q> solutions = getQConfigsRRT(bottleFrame, robotUR5, wc, state);
			...
			if (!collisionFreeSolutions.empty())
			{
				...
				RRTp2pPlanning(start_q, collisionFreeSolutions[0], total_length, total_time, robotUR5, detector, tStatePath, time, state, frameTcp);
				rw::math::Transform3D<> temp_transform = bottleFrame->getTransform(state);
				bottleFrame->setTransform(end_transform, state);

				std::vector<rw::math::Q> solutions_end = getQConfigsRRT(bottleFrame, robotUR5, wc, state);
				...
				{
					RRTp2pPlanning(collisionFreeSolutions[0], collisionFreeSolutionsEnd[0], total_length, total_time, robotUR5, detector, tStatePath, time, state, frameTcp);
				}
	...
	saveDataToCSVPlanning(placement1_total_times, "../csv_data/P1_RRT_time.csv");
	saveDataToCSVPlanning(placement2_total_times, "../csv_data/P2_RRT_time.csv");
	saveDataToCSVPlanning(placement3_total_times, "../csv_data/P3_RRT_time.csv");
	saveDataToCSVPlanning(placement1_total_length, "../csv_data/P1_RRT_length.csv");
	saveDataToCSVPlanning(placement2_total_length, "../csv_data/P2_RRT_length.csv");
	saveDataToCSVPlanning(placement3_total_length, "../csv_data/P3_RRT_length.csv");
